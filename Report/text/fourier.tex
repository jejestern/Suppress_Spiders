...
\end{figure}
As before with the lines we have a look at what happens if we have the several of this beams, again equally spaced with a spacing of 20 pixels. We find that this image, lets call it $h(x)$, is a convolution of the image with the equally spaced lines ($f(x)$) and the image with the single beam ($g(x)$), namely
\begin{equation}
...
\subsection{Fourier Transformation of Point Sources}

\subsection{Simulation of Spyders}
