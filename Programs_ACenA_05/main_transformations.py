...
path = "/home/jeje/Dokumente/Masterthesis/Programs_ACenA_05/ACenA_05"
files = os.listdir(path)


for image_name in files[0:2]:
    if image_name.endswith("1.fits"): 
        ...
        plt.imshow(int1*mask, origin='lower', cmap='gray', vmin=0, vmax=I_max)
        plt.colorbar()
        plt.tight_layout()
        ...
        im = ax.imshow(warped_or, origin='lower', aspect=aspect_value, vmin=0, 
                       vmax=I_max, extent=[0, 360, R_1, R_2])
        ...
        plt.imshow(h2, origin='lower', cmap='gray', vmin=0, vmax=I_max)
        plt.tight_layout()
        plt.colorbar()
        plt.show()
